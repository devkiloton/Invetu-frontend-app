...
const RadialChart = ({
  ...
  async function getSeries() {
    ...
    const ibov = getIbov();
    const portfolio = investments.map(stock => {
      ...
      const investimentsBeforeThisMonth = investments.filter(value => new Date(value.startDate).getTime() < dataStockThisMonth.firstDay.date * 1000);
      // join  them
      const joinedStockData = joinStockData(investimentsBeforeThisMonth)
      // take the value of each one
      const investimentsBeforeThisMonthValue = joinedStockData.map(value => dataStockThisMonth.firstDay.close * value.amount).reduce((accumulator, currentValue) => {
        return accumulator + currentValue
      },0);      
      // take the allocation of the current stock in the past month as a percent as string and convert to number
      const allocation = Number(getStockAllocation(stock.amount, dataStockThisMonth.firstDay.close, investimentsBeforeThisMonthValue));
      // multiply by the percent variation
      const variation = Number(getProfit(
  ...
  function getDataStocksThisMonth(
    ...
    const validData = history[0].historicalDataPrice.filter(h => h.date * 1000 >= timestampFirstDayMonth);
  ...
  useEffect(() => {
    ...
    getSeries().then(s => {
      ...
      setApexOptions({
        ...options, labels: [`Você: ${s[0].toFixed(2)}%`, `IBOV: ${s[1].toFixed(2)}%`, `CDI: ${s[2].toFixed(2)}%`]})
    });
